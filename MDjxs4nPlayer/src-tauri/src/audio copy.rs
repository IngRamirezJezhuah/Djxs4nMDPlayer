/*  AUDIO.RS
========================
modulo encargado de obtener los metadatos de lo que suena ahora mismo via
playerctl (puente a MPRIS/playerctl). tambien descarga y guarda la portada en
la carpeta public del proyecto para que el front la muestre.
 */
use std::io::Read;
...
impl EstadoReproductor {
    ...
    /// Alterna play/pausa. Devuelve true si quedo en reproduccion.
    pub fn altern_repro_pausar(&self) -> Result<bool, String> {
        Command::new("playerctl")
            ...
            .stderr(Stdio::null())
            .status()
            .map_err(|e| format!("Error jugando con playerctl: {}", e))?;

        Ok(Self::obtener_estado_reproduccion() == "Playing")
    }
    ...
    /// Junta todo lo que necesita el front relacionado a la cancion actual.
    ...
    // ejecuta un subcomando de playerctl y devuelve la salida como texto
    ...
    // lee titulo, artista, url cruda de la portada y estado
    ...
    // posicion actual en segundos (playerctl la imprime en segundos)
    ...
    // guarda la portada en la carpeta public del proyecto (public/ y su copia
    // servida por trunk en dist/public/) y devuelve la ruta relativa fija con
    // un query de version (?v=) basado en la fecha de modificacion para que el
    // front recargue la imagen cuando la caratula cambie.
    //
    // IMPORTANTE anti-bucle: si la portada que ya esta guardada es identica a
    // la nueva NO se vuelve a escribir nada. asi trunk no detecta cambios cada
    // segundo y no se mete en el loop infinito de recargar la app.
    fn preparar_portada(art_url: Option<String>) -> Option<String> {
        ...
        // si ya existe un cover.png con el mismo contenido no escribimos nada
    ...
    // ruta relativa fija con un ?v= que cambia con la fecha de modificacion
    ...
    // estado actual del reproductor activo
    fn obtener_estado_reproduccion() -> String {
        Self::playerctl("status", &[]).unwrap_or_else(|| "Detenido".to_string())
    }
}
